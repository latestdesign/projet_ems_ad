...
- Vous pouvez mettre du code R dans des chunks R et jouer sur les options comme
  ...
  + ...
- Vous pouvez mettre du code python dans des chunks python et jouer sur les options. Il ne faut pas oublier d'utiliser la librairie `reticulate` pour la compilation ([voir tutoriel](https://cmaugis.github.io/TutorielsR/Part4-Ravance.html#le-package-reticulate-pour-python)). Vous pouvez consulter la page web [https://rstudio.github.io/reticulate/](https://rstudio.github.io/reticulate/)  
...
Les données Iris ont été collectées par Edgar Anderson [@iris]. Ce sont les mesures en centimètres des variables suivantes : longueur du sépale (Sepal.Length), largeur du sépale (Sepal.Width), longueur du pétale (Petal.Length) et largeur du pétale (Petal.Width) pour trois espèces d'iris :*Iris setosa*, *I. versicolor* et *I. virginica*. Les données sont disponibles de base sous R et on les récupère donc avec la fonction `data(iris)`. Les premières lignes du jeu de données sont affichés dans la Table \ref{tab:tabiris}. 

```{r tabiris, echo=F}
data(iris)
library(knitr)
kable(head(iris),caption="\\label{tab:tabiris}Les premières lignes du jeu de données iris.")
```
...
Nous commençons par la variable *Species* (vecteur $Y$) qui est une variable qualitative. La figure \ref{fig:species} nous permet de contrôler que nous avons bien $50$ individus par espèce. 
...
Nous nous intéressons ici aux 4 variables quantitatives (matrice $X$). La figure \ref{fig:corrplot}  montre les corrélations entre les 4 variables. On peut remarquer que la largeur et la longuer des pétales sont fortement corrélées positivement, ce n'est pas le cas pour les sépales. 

```{r, echo=F,fig.cap="\\label{fig:corrplot}Matrice des corrélations entre les 4 variables quantitatives",fig.height=3}
...
On peut remarquer que les *Setosa* se distinguent des deux autres espèces principalement par la largeur de leur sépales. On peut appuyer ce point à l'aide de la figure \ref{fig:violin}.

```{r violiniris,echo=F,fig.height=3,message=F,fig.cap="\\label{fig:violin}Violin plots de la largeur des sépales pour chaque espèce."}
